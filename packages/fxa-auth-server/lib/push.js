...
const LOG_OP_PUSH_SEND_ERROR = "push.send.error";
  ...
  return {
    ...
    async sendPush(uid, devices, reason, options = {}) {
      ...
      if (devices.length > MAX_ACTIVE_DEVICES) {
        log.warning(LOG_OP_TOO_MANY_DEVICES, { uid });
      }
      for (const device of devices) {
        ...
        if (!device.pushCallback) {
          this.reportPushError(ERR_NO_PUSH_CALLBACK, metricsTags);
          continue;
        }
        if (device.pushEndpointExpired) {
          this.reportPushError(ERR_PUSH_CALLBACK_EXPIRED, metricsTags);
          continue;
        }
        ...
        if (options.data) {
          if (!device.pushPublicKey || !device.pushAuthKey) {
            this.reportPushError(ERR_DATA_BUT_NO_KEYS, metricsTags);
            continue;
          }
        ...
        try {
          ...
          if (
            ...
            this.reportPushError(ERR_PUSH_CALLBACK_RESET, metricsTags);
            ...
            try {
              ...
              log.warning(LOG_OP_DEVICE_UPDATE_FAILED, { uid, deviceId, err });
            }
          } else {
            this.reportPushError(err, metricsTags);
          }
        }
      }
    },
    ...
    reportPushError(err, metricsTags) {
      let errCode;
      ...
      this.incrementPushMetric(LOG_OP_PUSH_SEND_ERROR, metricsTags);
      log.error(LOG_OP_PUSH_SEND_ERROR, metricsTags);
    },
